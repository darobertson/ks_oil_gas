...
KS_oil_post1950_monthly.df <- data.frame(COUNTY=character())
for (fname in fnames) {
        fileToGet <- file.path(rawdatafolder, fname)
        ...
        KS_oil_post1950_monthly.df <- dplyr::bind_rows(KS_oil_post1950_monthly.df, df)
}

KS_oil_post1950_monthly.df <- dplyr::filter(KS_oil_post1950_monthly.df, !is.na(COUNTY))  # Remove rows where no county name
names(KS_oil_post1950_monthly.df) <- gsub("X.", "", names(KS_oil_post1950_monthly.df))
names(KS_oil_post1950_monthly.df) <- gsub("PRODUCTION", "PROD", names(KS_oil_post1950_monthly.df))
names(KS_oil_post1950_monthly.df) <- gsub("YEAR", "Year", names(KS_oil_post1950_monthly.df))
names(KS_oil_post1950_monthly.df) <- gsub("MONTH", "Month", names(KS_oil_post1950_monthly.df))
KS_counties <- unique(KS_oil_post1950_monthly.df$COUNTY)  # Extract a list of KS oil and gas producing counties


# Tidy the data into a long format
KS_oil_post1950_monthly.df <- tidyr::gather(KS_oil_post1950_monthly.df, "Measure", "Value", -c(COUNTY, Year, Month))  # Reshape to long format
KS_oil_post1950_monthly.df$COUNTY <- as.factor(KS_oil_post1950_monthly.df$COUNTY)
KS_oil_post1950_monthly.df$Value <- as.numeric(KS_oil_post1950_monthly.df$Value)
KS_oil_post1950_monthly.df$Measure <- as.factor(KS_oil_post1950_monthly.df$Measure)
KS_oil_post1950_monthly.df$Year <- as.character(KS_oil_post1950_monthly.df$Year)
